  ...
  const openModal = (biome, biomasData) => {
    ...
    biomeModal.innerHTML = `
      <div id="modalText">
        ...
        <h4>Fauna:</h4> <ul>${data.fauna.map(i => `<li>${i}</li>`).join("")}</ul>
        <h4>Flora:</h4> <ul>${data.flora.map(i => `<li>${i}</li>`).join("")}</ul>
    ...
    const closeModalFunc = () => {
      ...
      setTimeout(() => { biomeModal.style.display = "none"; biomeModal.innerHTML = ""; }, 300);
      document.removeEventListener("keydown", handleEsc);
    };
    const handleEsc = (e) => { if (e.key === "Escape") closeModalFunc(); };
    closeBtn.addEventListener("click", closeModalFunc);
    biomeModal.addEventListener("click", (e) => { if (e.target === biomeModal) closeModalFunc(); });
    document.addEventListener("keydown", handleEsc);
  };

  // --- CARREGAMENTO DE DADOS (PÁGINAS BIOMAS E GRÁFICO) ---
  const precisaDeDados = document.querySelector(".ver-detalhes") || document.getElementById('clima-content') || document.querySelector('.solo-card-container') || document.querySelector('.curiosidades-slider');
  if (precisaDeDados) {
    fetch("../dados.json")
    ...
    quizOptions.forEach(option => {
      option.addEventListener("click", () => {
        if (option.dataset.answer === "correct") {
          feedback.textContent = "✓ Correto! 'Tecnológico' não é um dos três pilares do desenvolvimento sustentável.";
          feedback.className = "quiz-feedback show correct";
        } else {
          feedback.textContent = "✗ Incorreto. Tente novamente! Os três pilares são Ambiental, Econômico e Social.";
  ...
  if (chartCanvas) {
    fetch("../dados.json")
        ...
        .then(data => {
            ...
            const detalheContainer = document.getElementById('detalhe-card-container');
            const detalheData = [
              { index: 0, titulo: "Mata Atlântica", cor: "#d90429", areas: "Principais áreas de desmatamento: Floresta de Araucárias, ecossistemas costeiros.", orgs: [{ nome: "SOS Mata Atlântica", link: "https://www.sosma.org.br/" }] },
              { index: 1, titulo: "Cerrado", cor: "#ef233c", areas: "Principais áreas de desmatamento: Formações savânicas para expansão agrícola.", orgs: [{ nome: "Instituto Cerrado", link: "https://www.institutocerrado.org.br/" }] },
              { index: 2, titulo: "Amazônia", cor: "#8d99ae", areas: "Principais áreas de desmatamento: Formações florestais para agropecuária e extração.", orgs: [{ nome: "WWF Brasil", link: "https://www.wwf.org.br/participe/doe/" }] },
              { index: 3, titulo: "Pampa", cor: "#43aa8b", areas: "Principais áreas de desmatamento: Campos nativos para agricultura.", orgs: [{ nome: "The Nature Conservancy Brasil", link: "https://www.tnc.org.br/" }] },
              { index: 4, titulo: "Pantanal", cor: "#2b9348", areas: "Principais áreas de desmatamento: Áreas úmidas afetadas por queimadas e pecuária.", orgs: [{ nome: "WWF Brasil", link: "https://www.wwf.org.br/participe/doe/" }] },
              { index: 5, titulo: "Caatinga", cor: "#f9c74f", areas: "Principais áreas de desmatamento: Vegetação nativa para lenha e agricultura.", orgs: [{ nome: "Instituto Cerrado", link: "https://www.institutocerrado.org.br/" }] }
            ];
            ...
            document.querySelectorAll('.data-point').forEach(point => {
              point.addEventListener('click', () => {
                ...
                if (detalhe) {
                  let orgList = detalhe.orgs.map(org => `<li><a href="${org.link}" target="_blank">${org.nome}</a></li>`).join('');
                  detalheContainer.innerHTML = `
